...
const ghCommands = {
  ...
  listLabels: (repo: string) => `gh label list --repo "${repo}" --json name`,
  createLabel: (repo: string, label: Label) =>
    `gh label create "${label.name}" --color "${
      label.color
    }" --repo "${repo}" ${
...
const execGhCommand = async (cmd: string): Promise<string> => {
  try {
...
const ensureLabelsExist = async (
  ...
  const existingLabels = new Set(
    JSON.parse(stdout).map((l: { name: string }) => l.name)
  );
...
export const getIssueNumbers = async (repo: string): Promise<Issue[]> => {
...
export const transferIssue = async (
  sourceRepo: string,
  issues: Issue[],
  targetRepo: string
) => {
  ...
  const issueMap = new Map<number, number[]>(
    issues
      .map(
        (issue) =>
          [issue.number, extractLinkedIssues(issue.body)] as [number, number[]]
      )
  ...
  const sortedIssues = [...issues].sort(
    (a, b) => Number(issueMap.has(a.number)) - Number(issueMap.has(b.number))
  );
  ...
  for (const issue of sortedIssues) {
    try {
      ...
      console.log(
        `Issue #${issue.number} transferred to ${targetRepo}${
          newNumber ? ` as #${newNumber}` : ""
        }`
      );
      ...
      if (issueMap.has(issue.number)) {
        console.log(
          `  - Transferred with sub-issues: #${issueMap
            .get(issue.number)
            ?.join(", #")}`
        );
      }
