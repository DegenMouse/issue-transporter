  ...
  .action(async (ownerArg, sourceRepoArg, targetRepoArg, options) => {
    ...
    try {
      ...
      if (!options.all) {
        ...
        console.log(
          "Transferring open issues (use --all to transfer both open and closed issues)"
        );
      } else {
      ...
      const { succesfull, unsuccesfull } = await transferIssue(
        sourceRepo,
        issuesToTransfer,
        targetRepo
      );
      succesfull.length > 0 &&
        console.log("Successfully transferred issues:", succesfull);
      unsuccesfull.length > 0 &&
        console.log("Unsuccessfully transferred issues:", unsuccesfull);
    } catch (error: any) {
